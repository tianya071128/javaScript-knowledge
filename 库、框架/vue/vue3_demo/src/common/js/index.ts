const tdist: { [K: string]: [string, string] } = {
  210184: ['沈北新区', '210100'],
  210185: ['其它区', '210100'],
  420381: ['丹江口市', '420300'],
  420383: ['其它区', '420300'],
...
export function getAreaList() {
  let province_list: Record<string, string> = {};
  let city_list: Record<string, string> = {};
  let county_list: Record<string, string> = {};
  getLev1().forEach((p) => {
    province_list[p.id] = p.text;
    getLev2(p.id).forEach((c) => {
      city_list[c.id] = c.text;
      getLev3(c.id).forEach((q) => (county_list[q.id] = q.text));
    });
  });
