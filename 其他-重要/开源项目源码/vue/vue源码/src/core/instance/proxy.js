 ...
 * @LastEditTime: 2020-03-26 21:18:35
 */
...
if (process.env.NODE_ENV !== "production") {
  ...
  const hasHandler = {
    has(target, key) {
      ...
      // isAllowed: 表示当访问了一个虽然不在实例对象上(或原型链上)的属性, 但如果你访问的是全局对象也是运行的.
      //            或者访问了以 _ 开头的属性,这么做是由于渲染函数中会包含很多以 _ 开头的内部方法, 如 _c、_v等
      const isAllowed =
        allowedGlobals(key) ||
        (typeof key === "string" &&
          key.charAt(0) === "_" &&
  ...
  // 为 initProxy 赋值 - initProxy作用: 设置渲染函数的作用域代理
